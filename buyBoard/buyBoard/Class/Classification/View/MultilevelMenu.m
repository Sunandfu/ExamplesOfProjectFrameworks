...
-(id)initWithFrame:(CGRect)frame WithData:(NSArray *)data withSelectIndex:(void (^)(NSInteger, NSInteger, id))selectIndex{
    if (self  == [super initWithFrame:frame]) {
        ...
        */
        ...
        [self.leftTablew registerNib:[UINib nibWithNibName:NSStringFromClass([MultilevelTableViewCell class]) bundle:nil] forCellReuseIdentifier:[MultilevelTableViewCell cellIdentifier]];
        /**
         右边的视图
         */
        ...
        UINib *nib=[UINib nibWithNibName:NSStringFromClass([MultilevelCollectionViewCell class]) bundle:nil];
        [self.rightCollection registerNib:nib forCellWithReuseIdentifier:[MultilevelCollectionViewCell cellIdentifier]];
        UINib *header=[UINib nibWithNibName:NSStringFromClass([CollectionHeader class]) bundle:nil];
        [self.rightCollection registerNib:header forSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:[CollectionHeader cellIdentifier]];
...
#pragma mark---左边的tablew deleagte
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    MultilevelTableViewCell * cell=[tableView dequeueReusableCellWithIdentifier:[MultilevelTableViewCell cellIdentifier]];
...
-(NSInteger)numberOfSectionsInCollectionView:(UICollectionView *)collectionView{
    if (self.allData.count==0) { return 0; }
...
-(NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section{
    ...
    if (title.nextArray.count>0) {
        ...
        //没有下一级
        if (sub.nextArray.count==0) return 1;
        else return sub.nextArray.count;
    } else {
        return title.nextArray.count;
    }
}
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    MultilevelCollectionViewCell *cell=[collectionView dequeueReusableCellWithReuseIdentifier:[MultilevelCollectionViewCell cellIdentifier] forIndexPath:indexPath];
    rightMeun * title=self.allData[self.selectIndex];
    NSArray * list;
    rightMeun * meun;
    meun=title.nextArray[indexPath.section];
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([kind isEqualToString: UICollectionElementKindSectionFooter ]){
        ...
        reuseIdentifier = [CollectionHeader cellIdentifier];
    }
    rightMeun *title = self.allData[self.selectIndex];
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath{
    return CGSizeMake(DEVICE_WIDTH-kLeftWidth, HEIGHT(70));
}
-(UIEdgeInsets)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout insetForSectionAtIndex:(NSInteger)section{
    return UIEdgeInsetsMake(0, 0, 0, 0);
}
-(CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout referenceSizeForHeaderInSection:(NSInteger)section{
    CGSize size={DEVICE_WIDTH-kLeftWidth,80};
    return size;
}
