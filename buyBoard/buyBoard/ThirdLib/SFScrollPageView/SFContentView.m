...
@interface SFContentView ()<UIScrollViewDelegate, UICollectionViewDelegate, UICollectionViewDataSource> {
    CGFloat   _oldOffSetX;
...
// 用于处理重用和内容的显示
@property (strong, nonatomic) SFCollectionView *collectionView;
// collectionView的布局
...
// 所有的子控制器
@property (strong, nonatomic) NSMutableDictionary<NSString *, UIViewController<SFScrollPageViewChildVcDelegate> *> *childVcsDic;
// 当前控制器
@property (strong, nonatomic) UIViewController<SFScrollPageViewChildVcDelegate> *currentChildVc;
...
- (void)commonInit {
    ...
    if ([navi isKindOfClass:[UINavigationController class]]) {
        ...
        if (navi.interactivePopGestureRecognizer) {
            ...
            [_collectionView setupScrollViewShouldBeginPanGestureHandler:^BOOL(SFCollectionView *collectionView, UIPanGestureRecognizer *panGesture) {
...
- (void)receiveMemoryWarningHander:(NSNotificationCenter *)noti {
    ...
    __weak typeof(self) weakSelf = self;
    [_childVcsDic enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, UIViewController<SFScrollPageViewChildVcDelegate> * _Nonnull childVc, BOOL * _Nonnull stop) {
        __strong typeof(self) strongSelf = weakSelf;
        if (strongSelf) {
            if (childVc != strongSelf.currentChildVc) {
                [self->_childVcsDic removeObjectForKey:key];
                [SFContentView removeChildVc:childVc];
            }
        }
...
- (void)reload {
    ...
    [self.childVcsDic enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, UIViewController<SFScrollPageViewChildVcDelegate> * _Nonnull childVc, BOOL * _Nonnull stop) {
        [SFContentView removeChildVc:childVc];
        childVc = nil;
...
- (void)willAppearWithIndex:(NSInteger)index {
    UIViewController<SFScrollPageViewChildVcDelegate> *controller = [self.childVcsDic valueForKey:[NSString stringWithFormat:@"%ld", index]];
    if (controller) {
        if ([controller respondsToSelector:@selector(sf_viewWillAppearForIndex:)]) {
            [controller sf_viewWillAppearForIndex:index];
        }
        if (_needManageLifeCycle) {
...
- (void)didAppearWithIndex:(NSInteger)index {
    UIViewController<SFScrollPageViewChildVcDelegate> *controller = [self.childVcsDic valueForKey:[NSString stringWithFormat:@"%ld", index]];
    if (controller) {
        if ([controller respondsToSelector:@selector(sf_viewDidAppearForIndex:)]) {
            [controller sf_viewDidAppearForIndex:index];
        }
        if (_needManageLifeCycle) {
...
- (void)willDisappearWithIndex:(NSInteger)index {
    UIViewController<SFScrollPageViewChildVcDelegate> *controller = [self.childVcsDic valueForKey:[NSString stringWithFormat:@"%ld", index]];
    if (controller) {
        if ([controller respondsToSelector:@selector(sf_viewWillDisappearForIndex:)]) {
            [controller sf_viewWillDisappearForIndex:index];
        }
        if (_needManageLifeCycle) {
...
- (void)didDisappearWithIndex:(NSInteger)index {
    UIViewController<SFScrollPageViewChildVcDelegate> *controller = [self.childVcsDic valueForKey:[NSString stringWithFormat:@"%ld", index]];
    if (controller) {
        if ([controller respondsToSelector:@selector(sf_viewDidDisappearForIndex:)]) {
            [controller sf_viewDidDisappearForIndex:index];
        }
        if (_needManageLifeCycle) {
            [controller endAppearanceTransition];
...
- (void)setupChildVcForCell:(UICollectionViewCell *)cell atIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(childViewController:forIndex:)]) {
        if (_currentChildVc == nil) {
            ...
            if (!_currentChildVc || ![_currentChildVc conformsToProtocol:@protocol(SFScrollPageViewChildVcDelegate)]) {
                NSAssert(NO, @"子控制器必须遵守SFScrollPageViewChildVcDelegate协议");
            }
            // 设置当前下标
            _currentChildVc.sf_currentIndex = indexPath.row;
    ...
    if (_currentChildVc.sf_scrollViewController != self.parentViewController) {
    ...
    if (_isLoadFirstView) { // 第一次加载cell? 不会调用endDisplayCell
        ...
        if (isFirstLoaded) {
            // viewDidLoad
            if ([_currentChildVc respondsToSelector:@selector(sf_viewDidLoadForIndex:)]) {
                [_currentChildVc sf_viewDidLoadForIndex:indexPath.row];
            }
        }
...
//    NSLog(@"消失消失:current:---- %ld   old ----- %ld indexpathRow----%ld ", _currentIndex, _oldIndex, indexPath.row);
    ...
    if (!self.forbidTouchToAdjustPosition) {
        if (_currentIndex == indexPath.row) {// 没有滚动完成
            ...
            if (_needManageLifeCycle) {
                UIViewController<SFScrollPageViewChildVcDelegate> *currentVc = [self.childVcsDic valueForKey:[NSString stringWithFormat:@"%ld", _oldIndex]];
                // 开始出现
                [currentVc beginAppearanceTransition:YES animated:NO];
                
                UIViewController<SFScrollPageViewChildVcDelegate> *oldVc = [self.childVcsDic valueForKey:[NSString stringWithFormat:@"%ld", indexPath.row]];
                // 开始消失
...
- (SFCollectionView *)collectionView {
    if (_collectionView == nil) {
        SFCollectionView *collectionView = [[SFCollectionView alloc] initWithFrame:self.bounds collectionViewLayout:self.collectionViewLayout];
...
- (NSMutableDictionary<NSString *,UIViewController<SFScrollPageViewChildVcDelegate> *> *)childVcsDic {
    if (!_childVcsDic) {
