...
@interface SFScrollSegmentView ()<UIScrollViewDelegate> {
...
@implementation SFScrollSegmentView
...
- (void)titleLabelOnClick:(UITapGestureRecognizer *)tapGes {
    ...
    SFTitleView *currentLabel = (SFTitleView *)tapGes.view;
...
- (void)setupTitles {
    ...
    for (NSString *title in self.titles) {
        ...
        SFTitleView *titleView = [[SFTitleView alloc] initWithFrame:CGRectZero];
        titleView.tag = index;
...
- (void)setupUI {
    ...
    if (self.segmentStyle.isScrollTitle) { // 设置滚动区域
        SFTitleView *lastTitleView = (SFTitleView *)self.titleViews.lastObject;
...
- (void)setupScrollViewAndExtraBtn {
    ...
    //        scrollW = maxX;
    //    }
    self.scrollView.frame = CGRectMake(0.0, 0.0, scrollW, self.sf_height);

    if (self.extraBtn) {
        self.extraBtn.frame = CGRectMake(scrollW , extraBtnY, extraBtnW, self.sf_height - 2*extraBtnY);
    }
}
...
- (void)setUpTitleViewsPosition {
    ...
    CGFloat titleY = 0.0;
    CGFloat titleW = 0.0;
    CGFloat titleH = self.sf_height - self.segmentStyle.scrollLineHeight;
    
    if (!self.segmentStyle.isScrollTitle) {// 标题不能滚动, 平分宽度
        titleW = self.scrollView.bounds.size.width / self.titles.count;
        
        NSInteger index = 0;
        for (SFTitleView *titleView in self.titleViews) {
    ...
    SFTitleView *currentTitleView = (SFTitleView *)self.titleViews[_currentIndex];
...
- (void)setupScrollLineAndCover {
    ...
    SFTitleView *firstLabel = (SFTitleView *)self.titleViews[0];
    CGFloat coverX = firstLabel.sf_x;
    CGFloat coverW = firstLabel.sf_width;
    ...
    if (self.scrollLine) {
        ...
        if (self.segmentStyle.isScrollTitle) {
            self.scrollLine.frame = CGRectMake(coverX , self.sf_height - self.segmentStyle.scrollLineHeight, coverW , self.segmentStyle.scrollLineHeight);

        } else {
            if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                coverW = [self.titleWidths[_currentIndex] floatValue] + wGap;
                coverX = (firstLabel.sf_width - coverW) * 0.5;
            }
...
- (void)adjustUIWhenBtnOnClickWithAnimate:(BOOL)animated taped:(BOOL)taped {
    ...
    SFTitleView *oldTitleView = (SFTitleView *)self.titleViews[_oldIndex];
    SFTitleView *currentTitleView = (SFTitleView *)self.titleViews[_currentIndex];
    ...
    [UIView animateWithDuration:animatedTime animations:^{
        ...
        if (weakSelf.scrollLine) {
            if (weakSelf.segmentStyle.isScrollTitle) {
                weakSelf.scrollLine.sf_x = currentTitleView.sf_x;
                weakSelf.scrollLine.sf_width = currentTitleView.sf_width;
            } else {
                if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                    CGFloat scrollLineW = [self.titleWidths[self->_currentIndex] floatValue] + wGap;
                    CGFloat scrollLineX = currentTitleView.sf_x + (currentTitleView.sf_width - scrollLineW) * 0.5;
                    weakSelf.scrollLine.sf_x = scrollLineX;
                    weakSelf.scrollLine.sf_width = scrollLineW;
                } else {
                    weakSelf.scrollLine.sf_x = currentTitleView.sf_x;
                    weakSelf.scrollLine.sf_width = currentTitleView.sf_width;
                }
        ...
        if (weakSelf.coverLayer) {
            if (weakSelf.segmentStyle.isScrollTitle) {
                ...
                weakSelf.coverLayer.sf_x = currentTitleView.sf_x - xGap;
                weakSelf.coverLayer.sf_width = currentTitleView.sf_width + wGap;
            } else {
                if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                    CGFloat coverW = [self.titleWidths[self->_currentIndex] floatValue] + wGap;
                    CGFloat coverX = currentTitleView.sf_x + (currentTitleView.sf_width - coverW) * 0.5;
                    weakSelf.coverLayer.sf_x = coverX;
                    weakSelf.coverLayer.sf_width = coverW;
                } else {
                    weakSelf.coverLayer.sf_x = currentTitleView.sf_x;
                    weakSelf.coverLayer.sf_width = currentTitleView.sf_width;
                }
            }
...
- (void)adjustUIWithProgress:(CGFloat)progress oldIndex:(NSInteger)oldIndex currentIndex:(NSInteger)currentIndex {
    ...
    SFTitleView *oldTitleView = (SFTitleView *)self.titleViews[oldIndex];
    SFTitleView *currentTitleView = (SFTitleView *)self.titleViews[currentIndex];

    
    CGFloat xDistance = currentTitleView.sf_x - oldTitleView.sf_x;
    CGFloat wDistance = currentTitleView.sf_width - oldTitleView.sf_width;
    
    if (self.scrollLine) {
        
        if (self.segmentStyle.isScrollTitle) {
            self.scrollLine.sf_x = oldTitleView.sf_x + xDistance * progress;
            self.scrollLine.sf_width = oldTitleView.sf_width + wDistance * progress;
        } else {
            if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                ...
                CGFloat oldScrollLineX = oldTitleView.sf_x + (oldTitleView.sf_width - oldScrollLineW) * 0.5;
                CGFloat currentScrollLineX = currentTitleView.sf_x + (currentTitleView.sf_width - currentScrollLineW) * 0.5;
                xDistance = currentScrollLineX - oldScrollLineX;
                self.scrollLine.sf_x = oldScrollLineX + xDistance * progress;
                self.scrollLine.sf_width = oldScrollLineW + wDistance * progress;
            } else {
                self.scrollLine.sf_x = oldTitleView.sf_x + xDistance * progress;
                self.scrollLine.sf_width = oldTitleView.sf_width + wDistance * progress;
            }
    ...
    if (self.coverLayer) {
        if (self.segmentStyle.isScrollTitle) {
            self.coverLayer.sf_x = oldTitleView.sf_x + xDistance * progress - xGap;
            self.coverLayer.sf_width = oldTitleView.sf_width + wDistance * progress + wGap;
        } else {
            if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                ...
                CGFloat oldCoverX = oldTitleView.sf_x + (oldTitleView.sf_width - oldCoverW) * 0.5;
                CGFloat currentCoverX = currentTitleView.sf_x + (currentTitleView.sf_width - currentCoverW) * 0.5;
                xDistance = currentCoverX - oldCoverX;
                self.coverLayer.sf_x = oldCoverX + xDistance * progress;
                self.coverLayer.sf_width = oldCoverW + wDistance * progress;
            } else {
                self.coverLayer.sf_x = oldTitleView.sf_x + xDistance * progress;
                self.coverLayer.sf_width = oldTitleView.sf_width + wDistance * progress;
            }
        }
    }
...
- (void)adjustTitleOffSetToCurrentIndex:(NSInteger)currentIndex {
    ...
    int index = 0;
    for (SFTitleView *titleView in _titleViews) {
...
//
    ...
    if (self.scrollView.contentSize.width != self.scrollView.bounds.size.width + contentSizeXOff) {// 需要滚动
        SFTitleView *currentTitleView = (SFTitleView *)_titleViews[currentIndex];
        ...
        CGFloat extraBtnW = self.extraBtn ? self.extraBtn.sf_width : 0.0;
